...
namespace KwizzApi.Utils
{
    ...
    public static class DockerNameGenerator
    {
        ...
        {
            return GetRandomName(Rng);
        }

        private static string GetRandomName(Random rng)
        {
            var leftIdx = rng.Next(left.Length);
            var rightIdx = rng.Next(right.Length);

            return string.Join("-", left[leftIdx], right[rightIdx]);
        }
